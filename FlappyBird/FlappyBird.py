...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  6 15:00:47 2023
...
# here we setup are game 
SW = 280  #SW  window Width
SH = 511 #SH window Height
...
BIRD = 'bird1.png'  #Here we can change the bird image 
BG = 'bg.png'  #here we can change the background image
PIPE = 'pipe.png'  #here we can change the pipe image
...
def game_start(generation,x,y):
	...
	up_pipes = [
	{'x':SW +200,'y': newPipe1[0]['y']},   #Here we setup the height and width if the pipes
	...
	birdyvel = -9    #Here we setup the velocity of bird and pipes
	birdymaxvel = 10
	birdyvelmin = -8
	birdyacc = 1
	...
	while(True):
		...
		if crashTest:
			reward = -1000   #here we setup the reward function if touches the pipe get -1000 reward
			Q_update(x_prev,y_prev,jump,reward,x_new,y_new)
			return score

		reward = 15  #if it passes the pipe then get reward of 15
		...
		pygame.display.update()
		FPSCLOCK.tick()
#Here we setup are Q learning algorithm using RL
...
if __name__=="__main__":
	...
	IMAGES['base'] = pygame.image.load('base.png').convert_alpha()  #Here also we have to update the base image if we change
